...
public class MultiFileContentSplitter implements ContentSplitter {
    public static class Builder {
        ...
        public ContentSplitter build() {
            return new MultiFileContentSplitter(this.maxPartCharacters);
        }
    }
    ...
    private MultiFileContentSplitter(int maxPartCharacters) {
        this.maxPartCharacters = maxPartCharacters;
    }
    ...
    public List<List<ContentItem>> splitContent(Iterable<ContentItem> items) {
        final int totalLength = StreamSupport.stream(items.spliterator(), false)
            .collect(Collectors.summingInt(MultiFileContentSplitter::getSpokenCharacterCount));
